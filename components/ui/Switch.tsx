...
const SwitchNative = React.forwardRef<SwitchPrimitives.RootRef, SwitchPrimitives.RootProps>(
  ({className, ...props}, ref) => {
    ...
    const animatedRootStyle = useAnimatedStyle(() => {
      return {
        backgroundColor: interpolateColor(translateX.value, [0, 18], [Colors.secondary_rgb, Colors.accent_rgb]),
      };
    });
          ...
          <Animated.View style={animatedThumbStyle}>
            <SwitchPrimitives.Thumb
              className={`h-7 w-7 rounded-full ${
                props.checked ? "bg-foreground" : "bg-secondary-foreground"
              } shadow-md shadow-foreground/25 ring-0`}
            />
          </Animated.View>
        </SwitchPrimitives.Root>
      </Animated.View>
